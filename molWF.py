...
def _checkPatientID(patientID,studyID='101',allowedSuffix=['FAM']):
	...
	if tmp[0] != studyID:
		...
		raise ValueError ("ERR:0002 unexpected member id "+'-'.join(tmp[2:]))
...
def catFM(fMNames,sampleIDs,foutPath,studyID='101'):
	...
	for finName in fMNames:
		print 'WARN0001: not checking member status, logic assumes all columns of the feature matrix at '+finName+' can be mapped to -FAM'
